...
// Custom Interfaces
export interface ITelegramJSONInline {
...
export function addAdditionalFields(
	...
	if (['inlineKeyboard', 'replyKeyboard', 'inlineKeyboardJSON'].includes(replyMarkupOption)) {
		...
		console.log('replyMarkupOption = ', replyMarkupOption);
		console.log('keyboardData.rows = ', keyboardData.rows);

		if (replyMarkupOption === 'inlineKeyboardJSON') {
			console.log('Работаем');
			const kb = JSON.parse(this.getNodeParameter(replyMarkupOption, index) as string);
			console.log('kb =', kb)
			const inlineMenu = {
				"inline_keyboard":kb
			}
			body.reply_markup  =  inlineMenu as IDataObject;
			console.log('body.reply_markup = ', body.reply_markup)
			return
		}
